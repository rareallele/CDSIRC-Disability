...
```{r}
disability = data %>% filter(disability_register == "Yes")
```


```{r}
disability_long = pivot_longer(disability, 
                               cols = c(disability_categories___1:disability_categories___9),
                               names_to = "Disability_category",
                               values_to = "Yes_No") %>% 
  filter(Yes_No == "Checked") %>% 
  mutate(Yes_No = recode(Yes_No, Checked = "Yes"))
...
yearly = disability_long %>%
  ...
  group_by(year_of_death, Disability_category) %>% 
  tally()  
```
...
yearly_percentage = data %>% 
  group_by(year_of_death, disability_register) %>% 
  tally() %>% 
  pivot_wider(id_cols = year_of_death, names_from = disability_register, values_from = n) %>% 
  mutate(Pending = replace_na(Pending, 0)) %>% 
  mutate(Total = Yes + No + Pending,
...
(yearly_deaths = plot_ly(
        ...
        type = "scatter",
        mode = "line",
        hovertemplate = paste0("Year: ", yearly_percentage$year_of_death, "\n",
                               "Total deaths: ", yearly_percentage$Total, "\n",
                               "Number of deaths with disability: ", yearly_percentage$Yes, "\n",
...
saveWidget(partial_bundle(yearly_deaths), "Blog/Figures/yearly_deaths.html", selfcontained = F, libdir = "index_files")
```
...
```{r}
disability_by_age_group = data %>% 
  group_by(`Age Group`, `Disability Register`) %>% 
  summarise(count = n()) %>% 
  left_join(
    data %>% group_by(`Age Group`) %>% summarise(total = n()),
    on = "Age Group"
  ) %>% 
  mutate(percentage = round(count/total*100,1))
#write_csv(disability_by_age_group, "disability_precentage_agegroup.csv")

age_group_percentage = disability %>% group_by(`Age Group`) %>% 
  summarise(percentage = n()/nrow(disability)*100)
```
